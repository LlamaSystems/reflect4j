...
public interface AnnotatedElementDescriptor<T extends AnnotatedElement, SELF extends AnnotatedElementDescriptor<T, SELF>>
    ...
    ///
    /// If the signature does not start with the '@' character, an [InvalidSignatureException]
    ...
    /// @throws InvalidSignatureException if the signature does not start with "@"
    ...
    ///
    /// If the signature does not start with `@`, an [InvalidSignatureException] is thrown.
    ///
    /// @param signature the fully qualified annotation class name; must not be `null`
